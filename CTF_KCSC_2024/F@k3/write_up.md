...
- Câu hỏi to lớn nhất đầu tiên trong đầu của tui lúc bấy giờ là chức năng của hàm `function_1` và `function_2` là gì??? (sau khi nghe bạn tui làm được bài này nói thì đây là 2 hàm là `CR4_init` và `CR4_encrypt`). Nhưng câu hỏi tui muốn đặt ra là nếu như tui không biết về `CR4` thì có thể hiểu được chức năng của 2 hàm này không?? (sau đoạn này xin phép đổi lại tên `function_1` thành `CR4_init` và `function_2` thành `CR4_encypt` nhưng ta vẫn chưa biết nhiệm vụ của chúng là gì, nên bây giờ là những suy nghĩ của tui).

- Bắt đầu với `CR4_init`.

    ![alt text](_IMG/Fake/image-8.png)

- Tham số truyền vào hàm `CR4_init` là mảng `map`, chuỗi `Str` (F@**!), và số nguyên `Str_len` (5).
...
- Vậy tóm lại với góc nhìn của tui thì hàm `CR4_init` là một hàm đổi chỗ các giá trị trong một mảng có `256` phần từ với các giá trị phần tử lần lượt từ `0` đến `255`, và hàm này là một hàm 'cố định' (tức là hàm sẽ trả về một mảng sau khi đổi chỗ là như nhau nếu tui có thay đổi input như thế nào, vì `Str` là chuỗi cố định ban đầu là `F@**!`).

- Tiếp theo với hàm `CR4_encrypt`.

    ![alt text](_IMG/Fake/image-9.png)

- Tham số truyền vào hàm `CR4_encrypt` là mảng `map`, mảng `cipher`, số nguyên `cipher_len` (27), mảng `String2`.
